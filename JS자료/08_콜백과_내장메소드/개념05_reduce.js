...
console.log([1, 2, 3, 4, 5].reduce((acc, n) => acc + n, 10));
...
console.log("직접 해보기4");
console.log(scores.reduce((acc, n) => (n < acc ? n : acc), scores[0]));
console.log(scores.reduce((acc, n) => (n < acc ? n : acc)));
// 현재값 생략하면 초기값을 적용함
...
console.log("직접 해보기 5");
console.log(
  ["A", "B", "A", "A"].reduce((acc, item5) => {
    acc[item5] = (acc[item5] ?? 0) + 1;
    return acc;
  }, {}),
);
