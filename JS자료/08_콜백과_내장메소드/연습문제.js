...
nums3.forEach((nums) => console.log(nums));
...
const names8 = users8.map((user) => user.name);
console.log(names8);
...
console.log(
  products10.filter((item) => item.stock === 0).map((item) => item.name),
);
...
console.log(
  products10.toSorted((a, b) => a.price - b.price).map((p) => p.name),
);
...
const stockk = products10
  .filter((a) => a.stock > 0)
  .reduce((acc, item) => acc + item.price * item.stock, 0);

console.log(`재고 자산 ${stockk}원`);
...
const counts2 = votes19.reduce((acc, item) => {
